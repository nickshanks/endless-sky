...
void ShopPanel::Draw()
{
	...
	if(isDraggingShip)
	{
		for(const auto &[ship, offset] : dragOffsets)
		{
			...
			DrawShipIcon(*ship, drawPoint, selected, swizzle);
		}
	}
...
void ShopPanel::DrawShip(const Ship &ship, const Point &center, bool isSelected, bool isOwned) const
{
	...
	if(sprite)
	{
		...
		DrawThumbnail(thumbnail, true, center + nudge, zoom, swizzle);
	}
...
set<int> ShopPanel::GetSelectedIndices() const
{
	set<int> indices;
...
bool ShopPanel::ReorderShipsTo(int toIndex)
{
	...
	toIndex = min(toIndex + direction, static_cast<int>(ships.size()));
...
void ShopPanel::SideSelect(int count)
{
	...
	SideSelect(shipStacks[newIndex]);
}
